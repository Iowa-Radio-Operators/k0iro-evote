"""
Client Application Authentication Integration for eVoting
...
THIS_APP_URL = os.getenv('THIS_APP_URL', 'https://evote.k0iro.com')
...
def init_session_from_token(token, validate_remote=False):
    ...
    # Set up session (matching your existing session structure)
    session['user_id'] = payload['user_id']
    session['user'] = payload['callsign']  # Your app uses 'user' not 'callsign'
...
# =====================================================
# VOTER PERMISSION CHECK
# =====================================================

def voter_required(f):
    """Decorator: Require user to have voter role"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not session.get('authenticated'):
            session['next_page'] = request.url
        ...
        if 'voter' not in user_roles and 'admin' not in user_roles:
            return "Access denied - voter permission required", 403
